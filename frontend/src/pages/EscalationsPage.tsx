    ...
    <div className="flex-1 min-w-0 overflow-y-auto">
      <div className="px-6 pt-6 pb-10">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          <div className="text-[11px] uppercase tracking-widest text-slate-400">Human in the loop</div>
        ...
        {isLoading && <div className="mt-6 text-xs text-slate-400">Loading escalations...</div>}
        {!isLoading && items.length === 0 && (
          <div className="mt-8 rounded-xl ring-1 ring-white/10 bg-white/[0.03] p-6 text-center text-sm text-slate-400">
          ...
          {items.map((i, idx) => {
            ...
            return (
              <motion.button
                ...
                className={`w-full text-left rounded-xl ring-1 ${sev.border} ${sev.bg} p-4 flex items-center gap-4 transition-colors hover:bg-white/[0.06]`}
              >
                <div className="w-10 h-10 rounded-lg ring-1 ring-white/15 grid place-items-center shrink-0" style={{ color: sev.icon }}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-[10px] text-slate-400">{i.external_id}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full uppercase tracking-widest ring-1 ${sev.border} ${sev.text}`}>
                      {i.severity}
                    </span>
                  </div>
                  <div className="text-xs text-slate-300 mt-1 line-clamp-2">{i.reason}</div>
                </div>
